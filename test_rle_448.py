...
model = PFLDInference()
model.load_state_dict(torch.load('weights/pfld_rle_3_24.pth', map_location='cpu'))
...
def four_point_transform(image, pts):
    ...
    warped = cv2.warpPerspective(image, M, (maxWidth, maxHeight), borderValue=(175, 175, 175),flags=cv2.INTER_AREA)
    return warped
...
for file in files[0:50]:
    ...
    #旋转
    if direction==2:
        frame_transformed = cv2.rotate(frame_transformed, cv2.ROTATE_90_COUNTERCLOCKWISE)  # 逆时针旋转
    elif direction==3:
        frame_transformed = cv2.rotate(frame_transformed, cv2.ROTATE_180)  # 180
    elif direction==4:
        frame_transformed = cv2.rotate(frame_transformed, cv2.ROTATE_90_CLOCKWISE)  # 顺时针旋转
